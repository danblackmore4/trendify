# Downloads the example images from trends_summary.json so they can be viewed locally.
# Only processes the top 10 garment types by post count to avoid downloading everything.
# Instagram CDN URLs expire after a few hours, so HTTP 403/404 errors on older exports are normal.
...
def slugify(text: str) -> str:
    # Turns "wide-leg trousers" into "wide-leg_trousers" — safe for use in filenames
...
def build_filename(garment_type: str, username: str, index: int, ext: str) -> str:
    ...
    # Double underscore separates garment type from username so neither can be mistaken for the other
...
def download_image(url: str, dest_path: str):
    ...
    with urllib.request.urlopen(req, timeout=30) as response:
        ...
        if ext in (".jpe", ".jpeg"):
            ext = ".jpg"
        # We pass a .jpg placeholder path but correct it here once we know the real extension
...
def main():
    ...
    # trends_summary.json is already sorted by post_count, but we sort here too
    # so this script works correctly even if the file was modified manually
    ...
    for garment in garment_types:
        ...
        # Track how many times we've seen each garment+username combo so we can
        # append a numeric suffix if the same influencer appears more than once
